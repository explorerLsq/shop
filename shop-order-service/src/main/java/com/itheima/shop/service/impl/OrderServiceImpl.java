...
public class OrderServiceImpl implements IOrderService {
    ...
    public Result confirmOrder(TradeOrder order) {
        ...
        try {
           ...
           // CastException.cast(ShopCode.SHOP_FAIL);
            ...
            //1.确认订单失败,发送消息
            //订单ID 优惠券ID 用户ID 余额  商品ID  商品数量  -封装成实体类
    ...
    private void reduceGoodsNum(TradeOrder order) {
        //参数可以是：订单ID，商品ID，商品数量
        //因为在同一个数据表中存在，所以使用mapper对象传递
    ...
    private Long savePreOrder(TradeOrder order) {
        ...
        //2.设置订单ID,为防止后期数据库分库分表，所以不使用自增ID，因为这样在分库分表时可能会产生ID冲突
    ...
    private void checkOrder(TradeOrder order) {
        ...
        //4.校验商品单价是否合法,校验的是订单中的商品单价和数据库中的商品单价是否一致
